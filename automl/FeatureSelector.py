...
class FeatureSelector():
    ...
    Implements five different methods to identify features for removal 
        ...
        1. Find collinear variables with a correlation greater than a specified correlation coefficient
        2. Find features with 0.0 feature importance from a gradient boosting machine (gbm)
        3. Find low importance features that do not contribute to a specified cumulative feature importance from the gbm
    ...
    --------
        ...
        target : array or series, default = None
            Array of target for training the machine learning model to find feature importances. These can be either binary target
            (if task is 'classification') or continuous targets (if task is 'regression').
            If no target are provided, then the feature importance based methods are not available.
    ...
    --------
        ...
        - All 43operations can be run with the `identify_all` method.
    ...
    def __init__(self, data, target=None):
        ...
        # Dataset and optional training target
        self.data = data
        self.target = target

        if target is None:
            print('No target provided. Feature importance based methods are not available.')
    ...
    def identify_zero_importance(self, task, eval_metric=None, 
        ...
        if self.target is None:
            raise ValueError("No training target provided.")
        ...
        for _ in range(n_iterations):
            ...
            if early_stopping:
                ...
                train_features, valid_features, train_target, valid_target = train_test_split(features, target, test_size = 0.15)

                # Train the model with early stopping
                model.fit(train_features, train_target, eval_metric = eval_metric,
    ...
    def identify_all(self, selection_params):
        ...
        selection_params : dict
           ...
           Params must contain the keys [ 'correlation_threshold', 'eval_metric', 'task', 'cumulative_importance']
        
        """
        
        # Check for all required parameters
        for param in [ 'correlation_threshold', 'eval_metric', 'task', 'cumulative_importance']:
    ...
    def remove(self, methods, keep_one_hot = True):
        ...
        --------
            methods : 'all' or list of methods
                ...
                Can be one of ['collinear', 'zero_importance', 'low_importance']
    ...
    def plot_collinear(self, plot_all = False):
        ...
        ax.set_yticktarget(list(corr_matrix_plot.index), size = int(160 / corr_matrix_plot.shape[0]));

        # Set the xtarget 
        ax.set_xticks([x + 0.5 for x in list(range(corr_matrix_plot.shape[1]))])
        ax.set_xticktarget(list(corr_matrix_plot.columns), size = int(160 / corr_matrix_plot.shape[1]));
    ...
    def plot_feature_importances(self, plot_n = 15, threshold = None):
        ...
        # Set the yticks and target
        ax.set_yticks(list(reversed(list(self.feature_importances.index[:plot_n]))))
        ax.set_yticktarget(self.feature_importances['feature'][:plot_n], size = 12)
