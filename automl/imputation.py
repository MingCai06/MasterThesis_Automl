...
class Imputation():
    ...
    def set_params(self, **params):
        ...
        for k, v in params.items():
            if k not in self.get_params():
                warnings.warn("Invalid parameter(s) for NA Imputation. "
                              "Parameter(s) IGNORED. "
                              "Check the list of available parameters with "
                              "`imputation.get_params().keys()`")
            else:
                setattr(self, k, v)
    ...
    def fit(self, df_train, y_train=None):
       ...
       """
        ...
        if (self.numerical_strategy in ['mean', 'median', "most_frequent"]):
            ...
            raise ValueError("Numerical strategy for NA Imputation is not valid")
    ...
    def fit_transform(self, df_train, y_train=None):
        ...
        """Fits NA Imputation and transforms the dataset.
        Parameters
        ----------
        df_train : pandas.Dataframe of shape = (n_train, n_features)
            The train dataset with numerical and categorical features.
